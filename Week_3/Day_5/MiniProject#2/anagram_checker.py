...
class AnagramChecker:
    def __init__(self):
        self.anagrams = [x[:-1] for x in open("Week_3\Day_5\MiniProject#2\sowpods.txt","r").readlines()]
    def is_valid_word(self , word):
        return word.upper() in self.anagrams
    def  get_anagrams(self , word):
        alist = []
        for i in self.anagrams:
            if sorted(list(i)) == sorted(list(word.upper())):
                alist.append(i)
        return alist